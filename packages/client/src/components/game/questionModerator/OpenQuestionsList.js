 ...
 *
 * `questions` is the single source of truth (comes straight from
 * ModeratorScreen's Redux-backed `mappedOpenQuestions`). We no longer copy
 * it into local state — the parent already re-fetches and filters the list
 * after a publish, so mirroring it here just created a second, stale copy.
 * See ModeratorScreen-code-audit.md, finding #5.
 */
export function OpenQuestionsList({ questions, onClose, onPublish }) {
