...
// Player invitation (white). DEMO ONLY — out of scope per the brief, kept so the
// designed multi-select screen stays visible. Self-contained state, nothing is
// sent to the server, and the step is skippable. Revisited next sprint.
export function PlayerInvitationStep({
  value,
  onChange,
  onNext,
  onBack,
  ...
  const toggle = (u) =>
    onChange(
      players.some((p) => p.id === u.id)
        ? players.filter((p) => p.id !== u.id)
        : [...players, u]
    );
  ...
  return (
    ...
    >
      <View style={styles.pane}>
        ...
        >
          <Text
            style={players.length ? styles.selectedName : styles.placeholder}
          >
            {players.length
              ? `${t('selectPlayers', 'Select players')} (${players.length})`
              : t('whoWillPlay', 'Who will be your players?')}
          </Text>
          <Ionicons
        ...
        {open ? (
            ...
            <ScrollView keyboardShouldPersistTaps="handled">
              {filterUsers(search).map((u) => (
                <UserPickerRow
                  key={u.id}
                  name={u.name}
                  selected={players.some((p) => p.id === u.id)}
                  onPress={() => toggle(u)}
                />
              ))}
            </ScrollView>
          ...
          players.map((p) => (
            <View key={p.id} style={styles.chipRow}>
              ...
              <TouchableOpacity onPress={() => toggle(p)}>
                <Ionicons
                  name="close"
                  size={20}
                  color={Colors.text.secondary}
